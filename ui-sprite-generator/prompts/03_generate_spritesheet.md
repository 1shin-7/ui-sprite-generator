...
The formal prompt artifact for Phase 3 must be generated by `scripts/build_spritesheet_prompt.py`. Do not handwrite the prompt, do not summarize the canonical prompt, and do not rewrite it into a shorter prompt.
...
- Use `spec.yaml` for structure, component ids, surface policy, occlusion reconstruction, and render intent.
...
Save only labeled atlas images under `atlas/`. The per-atlas crop map is produced later by `prompts/04_extract_atlas_map.md`.
