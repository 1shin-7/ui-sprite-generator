...
class BuildSpritesheetPromptTests(unittest.TestCase):
    ...
    def test_rejects_overpacked_sheet(self):
        module = load_script_module()
        spec = minimal_spec()

        with self.assertRaisesRegex(module.PromptBuildError, "exceeds max fill ratio"):
            module.build_prompt(spec, canvas_size="256x256", max_fill_ratio=0.65)
